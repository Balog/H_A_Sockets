...
if(LastCommand==Comm | LastCommand==0)
{
 switch(Comm)
 {
  case 1:
  {
   //ќтвет на команду запроса IP у клиента
   this->IP=Parameters[0];
   this->AppPatch=Parameters[1];
   //ѕередача сигнала готовности к дальнейшей работе
   //ќтвет на него не предусмотрен
   LastCommand=0;
  ...
  case 3:
  {
  //ќрганизовать регистрацию формы на сервере
//   ShowMessage(Parameters[0]);
   ...
   VForm.push_back(F);
   //ѕока передаем один и тот же параметр а должен передаватьс€ номер формы на сервере
