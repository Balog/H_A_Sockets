...
Comm->CommandText="UPDATE (Подразделения INNER JOIN Аспекты ON Подразделения.[Номер подразделения] = Аспекты.Подразделение) INNER JOIN ObslOtdel ON Подразделения.[Номер подразделения] = ObslOtdel.NumObslOtdel SET Аспекты.Del = True WHERE (((ObslOtdel.Login)="+IntToStr(NumLogin)+"));";
Comm->Execute();
...
{
 int N=TempAsp->FieldByName("Номер аспекта")->Value;
 ...
 else
 {
  ...
  TempAsp->Edit();
  TempAsp->FieldByName("Del")->Value=true;
  TempAsp->Post();
}
