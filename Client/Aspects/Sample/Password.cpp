...
Table* Logins=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Аспекты")].ServerDB);
...
{
 ShowMessage("Ошибка проверки пароля");
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Ошибка проверки пароля"," Ошибка "+IntToStr(GetLastError()));
}
}
else
{
...
Table* Group=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Аспекты")].ServerDB);
...
String LG=TLogin.SubString(1,TLogin.Length()/2);
 ...
 //ShowMessage("Логин проверен");
if(LG==TabLogin)
{
 Zast->MClient->SetLogin(TabLogin);
Zast->MClient->WriteDiaryEvent("NetAspects","Пароль принят","");
...
switch (Role)
{
 ...
 //ShowMessage("Переход к главспецу");
 ...
 {
 ShowMessage("Ошибка синхронизации логинов");
 Zast->Close();
 }
 break;
 }
 ...
 //ShowMessage("Переход к пользователю");
 Zast->MClient->SetLogin(TabLogin);
 Zast->MClient->Stop();
 Vvedenie->Show();
 this->Hide(); 
...
{
Zast->MClient->WriteDiaryEvent("NetAspects","Неверный пароль","Pass="+EdPass->Text);
Zast->MClient->DeleteTable(this, Group);
Zast->MClient->DeleteTable(this, Logins);
Zast->MClient->Stop();
Zast->Start=false;
