...
//NameForm(this, "Алгоритм оценки");
}
//---------------------------------------------------------------------------
String TMetodika::ReadMet()
{
Zast->MClient->WriteDiaryEvent("NetAspects","Начало загрузки методики","");
try
{
if(LoadMet())
{
...
ADODataSet1->CommandText="select * from Методика";
ADODataSet1->Active=true;
Zast->MClient->WriteDiaryEvent("NetAspects","Конец загрузки методики","");
return "Завершено";

}
else
{
Zast->MClient->WriteDiaryEvent("NetAspects","Сбой загрузки методики","");
return "Ошибка";
}
}
catch(...)
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Ошибка загрузки методики"," Ошибка "+IntToStr(GetLastError()));
return "Ошибка";
}
}
//-----------------------------------------------------------
...
{
Application->HelpFile=ExtractFilePath(Application->ExeName)+"NetAspects.HLP";
Application->HelpJump("IDH_АЛГОРИТМ_ОЦЕНКИ");
}
}
//---------------------------------------------------------------------------
