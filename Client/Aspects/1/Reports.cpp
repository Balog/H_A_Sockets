...
String S="Перечень экологических аспектов с "+Report1->Date1->Date.DateString()+" по "+Report1->Date2->Date.DateString();
App.OlePropertyGet("Cells",9,1).OlePropertySet("Value",S.c_str());
...
if(OneList)
{
Podrazd->CommandText="Select * from TempПодразделения Where Ver=true Order by [Название подразделения]";
}
else
{
Podrazd->CommandText="Select * from TempПодразделения Where Ver=true Order by [Название подразделения]  DESC";
}
}
else
{
if(this->Role==4)
{
Podrazd->CommandText="Select * from TempПодразделения Where [Номер подразделения]="+IntToStr(Podr)+" AND Ver=true  Order by [Название подразделения]  DESC";

}
else
{
Podrazd->CommandText="Select * from TempПодразделения Where ServerNum="+IntToStr(Podr)+" AND Ver=true  Order by [Название подразделения]  DESC";
}
}
Podrazd->Active=true;
int ListCount=1;
...
String S="Реестр значимых экологических аспектов с "+Report1->Date1->Date.DateString()+" по "+Report1->Date2->Date.DateString();
App.OlePropertyGet("Cells",9,1).OlePropertySet("Value",S.c_str());
