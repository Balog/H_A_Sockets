...
{
G="select * from TempAspects Where "+Filtr1+" Order By [Номер аспекта]";
}
...
{
G="SELECT "+T+" TempAspects.* FROM TempAspects INNER JOIN Подразделения ON TempAspects.Подразделение = Подразделения.[Номер подразделения] WHERE  "+Filtr1+" AND (((Подразделения.ServerNum)="+IntToStr(Podr)+")) ORDER BY TempAspects.[Номер аспекта];";
}
...
if (Filtr1=="")
{
G="select * from TempAspects Where Подразделение="+IntToStr(Podr)+" Order By [Номер аспекта]";
}
else
{
G="select * from TempAspects Where "+Filtr1+" AND Подразделение="+IntToStr(Podr)+" Order By [Номер аспекта]";
}
}
...
Table* RTemp=Zast->MClient->CreateTable(Report1, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Аспекты")].ServerDB);
...
Table* LMemo1=Zast->MClient->CreateTable(Report1, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Аспекты")].ServerDB);
...
Table* LMemo2=Zast->MClient->CreateTable(Report1, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Аспекты")].ServerDB);
...
Table* LMemo3=Zast->MClient->CreateTable(Report1, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Аспекты")].ServerDB);
...
Table* LMemo4=Zast->MClient->CreateTable(Report1, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Аспекты")].ServerDB);
