...
//Zast->BeginWork->Execute();
 ...
 if(Parameters[0]=="0")
 {
 //Команда на отключение клиента. В соединении отказано.

 ShowMessage("В соединении отказано. Клиент с таким логином уже подключен к серверу");
 Zast->Close();
 }
 else
 {
...
//  Zast->PrepareUpdateOtd->Execute();
 ...
 {
  //не удалять подразделение
  String S1="Это подразделение распределено для логина "+Parameters[0];
  String S=S1+"\rДля удаления этого подразделения администратор должен отсоединить его от логина";
 ...
 //Если таймер разблокировки уже выключен то это получается избыточный ответ сервера и не нужно ничего делать
 if(Zast->UnBlockServer->Enabled)
 {
 ...
 case 21:
 {
 //Команда аварийной разблокировки клиента
...
if(NameDB=="HReference")
{
DB=ADOConn;
}
if(NameDB=="Опасности_П")
{
DB=ADOUsrAspect;
}
