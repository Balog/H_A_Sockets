...
MClient->WriteDiaryEvent("Hazards","Запуск","");
...
int LicCount=MClient->GetLicenseCount("Опасности");
Reg=LicCount!=0;
...
this->Repaint();
Label2->Caption="Реорганизация архива... ";
this->Repaint();
*/
String Path=ExtractFilePath(Application->ExeName);
...
/*
Label2->Caption="Сжатие базы данных... ";
this->Repaint();
*/
ADOConn->PackDB();
ADOAspect->PackDB();
...
/*
Label2->Caption="Подключение к базе данных... ";
this->Repaint();
*/
ADOConn->ConnectDB();
ADOAspect->ConnectDB();
...
//CDBItem Item=VDB[CBDatabase->ItemIndex];
Table* RTable=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
//
...
//CDBItem Item=VDB[CBDatabase->ItemIndex];
Table* FromTable=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
//
