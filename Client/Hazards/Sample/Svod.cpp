...
SCount->CommandText="SELECT Count(Data.Z) AS [Count] FROM Data;";
...
LTemp->CommandText="SELECT TempAspects.[Номер аспекта], TempAspects.Подразделение, TempAspects.Ситуация, TempAspects.[Вид территории], TempAspects.Деятельность, TempAspects.Специальность, TempAspects.Аспект, TempAspects.Воздействие, TempAspects.G, TempAspects.O, TempAspects.R, TempAspects.S, TempAspects.T, TempAspects.L, TempAspects.N, TempAspects.Z, TempAspects.Значимость, TempAspects.[Наименование значимости], TempAspects.[Проявление воздействия], TempAspects.[Тяжесть последствий], TempAspects.Приоритетность,   TempAspects.[Дата создания], TempAspects.[Начало действия], TempAspects.[Конец действия] FROM TempAspects;";
...
for(SlReestr->First();!SlReestr->Eof;SlReestr->Next())
{
 ...
 {
  TempSvod->Insert();
  TempSvod->FieldByName("Название подразделения")->Value=D->FieldByName("Название подразделения")->Value;
  TempSvod->FieldByName("Наименование территории")->Value=D->FieldByName("Наименование территории")->Value;
  TempSvod->FieldByName("Наименование деятельности")->Value=D->FieldByName("Наименование деятельности")->Value;
  TempSvod->FieldByName("Наименование аспекта")->Value=D->FieldByName("Наименование аспекта")->Value;
  TempSvod->FieldByName("Специальность")->Value=D->FieldByName("Специальность")->Value;
  TempSvod->FieldByName("Наименование воздействия")->Value=D->FieldByName("Наименование воздействия")->Value;
  TempSvod->FieldByName("Название ситуации")->Value=D->FieldByName("Название ситуации")->Value;
  TempSvod->FieldByName("Z")->Value=D->FieldByName("Z")->Value;
  TempSvod->FieldByName("Наименование значимости")->Value=D->FieldByName("Наименование значимости")->Value;
  TempSvod->FieldByName("Наименование приоритетности")->Value=D->FieldByName("Наименование приоритетности")->Value;
  TempSvod->FieldByName("Выполняющиеся мероприятия")->Value=D->FieldByName("Выполняющиеся мероприятия")->Value;
  TempSvod->FieldByName("Предлагаемые мероприятия")->Value=D->FieldByName("Предлагаемые мероприятия")->Value;
  TempSvod->FieldByName("Мониторинг и контроль")->Value=D->FieldByName("Мониторинг и контроль")->Value;
  TempSvod->FieldByName("Предлагаемый мониторинг и контроль")->Value=D->FieldByName("Предлагаемый мониторинг и контроль")->Value;
...
//Реализовать запись данных в собственную таблицу MainSvodReestr
 ...
 {
  TempTable->Insert();
  TempTable->FieldByName("Название подразделения")->Value=TempSvod->FieldByName("Название подразделения")->Value;
  TempTable->FieldByName("Наименование территории")->Value=TempSvod->FieldByName("Наименование территории")->Value;
  TempTable->FieldByName("Наименование деятельности")->Value=TempSvod->FieldByName("Наименование деятельности")->Value;
  TempTable->FieldByName("Наименование аспекта")->Value=TempSvod->FieldByName("Наименование аспекта")->Value;
  TempTable->FieldByName("Специальность")->Value=TempSvod->FieldByName("Специальность")->Value;
  TempTable->FieldByName("Наименование воздействия")->Value=TempSvod->FieldByName("Наименование воздействия")->Value;
  TempTable->FieldByName("Название ситуации")->Value=TempSvod->FieldByName("Название ситуации")->Value;
  TempTable->FieldByName("Z")->Value=TempSvod->FieldByName("Z")->Value;
  TempTable->FieldByName("Наименование значимости")->Value=TempSvod->FieldByName("Наименование значимости")->Value;
  TempTable->FieldByName("Наименование приоритетности")->Value=TempSvod->FieldByName("Наименование приоритетности")->Value;
  TempTable->FieldByName("Выполняющиеся мероприятия")->Value=TempSvod->FieldByName("Выполняющиеся мероприятия")->Value;
  TempTable->FieldByName("Предлагаемые мероприятия")->Value=TempSvod->FieldByName("Предлагаемые мероприятия")->Value;
  TempTable->FieldByName("Мониторинг и контроль")->Value=TempSvod->FieldByName("Мониторинг и контроль")->Value;
  TempTable->FieldByName("Предлагаемый мониторинг и контроль")->Value=TempSvod->FieldByName("Предлагаемый мониторинг и контроль")->Value;
