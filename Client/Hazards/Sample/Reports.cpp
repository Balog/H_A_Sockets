...
if (Filtr1=="")
{
G1="SELECT "+T1+" TempAspects.* FROM TempAspects INNER JOIN Подразделения ON TempAspects.Подразделение = Подразделения.[Номер подразделения] WHERE (((Подразделения.ServerNum)="+IntToStr(Podr)+")) ORDER BY TempAspects.[Номер аспекта];";
}
else
{
G1="SELECT "+T1+" TempAspects.* FROM TempAspects INNER JOIN Подразделения ON TempAspects.Подразделение = Подразделения.[Номер подразделения] WHERE  "+Filtr1+" AND (((Подразделения.ServerNum)="+IntToStr(Podr)+")) ORDER BY TempAspects.[Номер аспекта];";
}
...
if (Filtr1=="")
{
G1="select * from TempAspects Where Подразделение="+IntToStr(Podr)+" Order By [Номер аспекта]";
}
else
{
G1="select * from TempAspects Where "+Filtr1+" AND Подразделение="+IntToStr(Podr)+" Order By [Номер аспекта]";
}
}
...
AnsiString Text;
int Num;
T="Перечень рисков c "+Date1.DateString()+" по "+Date2.DateString();
...
int Num;
AnsiString Ts;
//Ts="Реестр рисков профессиональных опасностей  c "+Date1.DateString()+" по "+Date2.DateString();
Ts="Реестр рисков профессиональных опасностей";
App.OlePropertyGet("Cells",9,1).OlePropertySet("Value",Ts.c_str());
TempAspects->Active=false;
TempAspects->CommandText="Select * From Подразделения Where [Номер подразделения]="+IntToStr(Podr);
TempAspects->Active=true;
NP=TempAspects->FieldByName("Название подразделения")->Value;
Ts=NP;
...
{
Number=i;
Num=Report->FieldByName("Вид территории")->Value;
TempAspects->Active=false;
TempAspects->CommandText="Select * From Территория Where [Номер территории]="+IntToStr(Num);
TempAspects->Active=true;
Text=TempAspects->FieldByName("Наименование территории")->Value;
...
App.OlePropertyGet("Cells",Start+Number+5,1).OlePropertySet("Value","должность");
...
App.OlePropertyGet("Cells",Start+Number+5,2).OlePropertySet("Value","Ф.И.О.");
App.OlePropertyGet("Cells",Start+Number+5,2).OlePropertyGet("Font").OlePropertySet("Size",8);
//App.OlePropertyGet("Range",(Address(Sheet,2,Start+Number+5)).c_str()).OlePropertySet("IndentLevel",7);

App.OlePropertyGet("Cells",Start+Number+5,3).OlePropertySet("Value","подпись");
App.OlePropertyGet("Cells",Start+Number+5,3).OlePropertyGet("Font").OlePropertySet("Size",8);
//App.OlePropertyGet("Range",(Address(Sheet,3,Start+Number+5)).c_str()).OlePropertySet("IndentLevel",7);

App.OlePropertyGet("Cells",Start+Number+6,1).OlePropertySet("Value","\"___\" _________ 20___ г");
...
if (Filtr2=="")
{
G="SELECT "+T+" TempAspects.* FROM TempAspects INNER JOIN Подразделения ON TempAspects.Подразделение = Подразделения.[Номер подразделения] WHERE (((Подразделения.ServerNum)="+IntToStr(Podr)+")) ORDER BY TempAspects.[Номер аспекта];";
}
else
{
G="SELECT "+T+" TempAspects.* FROM TempAspects INNER JOIN Подразделения ON TempAspects.Подразделение = Подразделения.[Номер подразделения] WHERE  "+Filtr2+" AND (((Подразделения.ServerNum)="+IntToStr(Podr)+")) ORDER BY TempAspects.[Номер аспекта];";
}
}
else
{
...
if(Filtr2=="")
{
G="Select * From TempAspects Where  Подразделение="+IntToStr(Podr)+" AND Значимость=True Order By [Номер аспекта]";
}
else
{
G="Select * From TempAspects Where "+Filtr2+" AND Подразделение="+IntToStr(Podr)+" AND Значимость=True Order By [Номер аспекта]";
}
}
...
AnsiString Text;
int Num;
T="Реестр значимых рисков с "+Date1.DateString()+" по "+Date2.DateString();
App.OlePropertyGet("Cells",9,1).OlePropertySet("Value",T.c_str());
TempAspects->Active=false;
TempAspects->CommandText="Select * From Подразделения Where [ServerNum]="+IntToStr(Podr);
TempAspects->Active=true;
...
AnsiString Text;
int Num;
//T="Реестр значимых рисков профессиональных опасностей с "+Date1.DateString()+" по "+Date2.DateString();
T="Реестр значимых рисков профессиональных опасностей";//+Date1.DateString()+" по "+Date2.DateString();
...
ZT=Report->FieldByName("Предлагаемые мероприятия")->Value;
while (ZT.Pos("\r"))
{
 int N=ZT.Pos("\r");
 AnsiString Z=ZT.Delete(N,1);
...
ZT=Report->FieldByName("Мониторинг и контроль")->Value;
while (ZT.Pos("\r"))
{
 int N=ZT.Pos("\r");
 AnsiString Z=ZT.Delete(N,1);
...
ZT=Report->FieldByName("Предлагаемый мониторинг и контроль")->Value;
while (ZT.Pos("\r"))
{
 int N=ZT.Pos("\r");
 AnsiString Z=ZT.Delete(N,1);
...
String Ts="Составил: _____________";
...
App.OlePropertyGet("Cells",Start+Number+3,1).OlePropertySet("Value","должность");
App.OlePropertyGet("Cells",Start+Number+3,1).OlePropertyGet("Font").OlePropertySet("Size",8);
//App.OlePropertyGet("Range",(Address(Sheet,1,Start+Number+5)).c_str()).OlePropertySet("IndentLevel",9);

App.OlePropertyGet("Cells",Start+Number+3,2).OlePropertySet("Value","Ф.И.О.");
App.OlePropertyGet("Cells",Start+Number+3,2).OlePropertyGet("Font").OlePropertySet("Size",8);
//App.OlePropertyGet("Range",(Address(Sheet,2,Start+Number+5)).c_str()).OlePropertySet("IndentLevel",7);

App.OlePropertyGet("Cells",Start+Number+3,3).OlePropertySet("Value","подпись");
...
App.OlePropertyGet("Cells",Start+Number+5,1).OlePropertySet("Value","Согласовано:");
App.OlePropertyGet("Range",(Address(Sheet,1,Start+Number+7)).c_str()).OlePropertyGet("Borders",9).OlePropertySet("Weight",2);
App.OlePropertyGet("Cells",Start+Number+8,1).OlePropertySet("Value","(должность)");
...
App.OlePropertyGet("Cells",Start+Number+11,1).OlePropertySet("Value","(должность)");
...
App.OlePropertyGet("Cells",Start+Number+8,3).OlePropertySet("Value","(подпись)");
...
App.OlePropertyGet("Cells",Start+Number+11,3).OlePropertySet("Value","(подпись)");
...
App.OlePropertyGet("Cells",Start+Number+8,5).OlePropertySet("Value","(Ф.И.О.)");
...
App.OlePropertyGet("Cells",Start+Number+11,5).OlePropertySet("Value","(Ф.И.О.)");
...
Table* RTemp=Zast->MClient->CreateTable(Report1, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Table* LMemo1=Zast->MClient->CreateTable(Report1, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Table* LMemo2=Zast->MClient->CreateTable(Report1, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Table* LMemo3=Zast->MClient->CreateTable(Report1, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Table* LMemo4=Zast->MClient->CreateTable(Report1, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
