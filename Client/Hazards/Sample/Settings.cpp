...
/*
Temp->Active=false;
int A=Podr->FieldByName("Номер подразделения")->Value;
Temp->CommandText="Select * From Аспекты Where Подразделение="+IntToStr(A);
Temp->Active=true;
if (Temp->RecordCount!=0)
{
AnsiString A="Вы уверены?\nУдаляемую запись использует "+IntToStr(Temp->RecordCount)+" аспектов!";
int Q=Application->MessageBoxA(A.c_str(),"Удаление подразделения",MB_YESNO);
if(Q==IDYES)
{
Podr->Delete();
}
...
/*
Temp->Active=false;
int A=Sit->FieldByName("Номер ситуации")->Value;
Temp->CommandText="Select * From Аспекты Where Ситуация="+IntToStr(A);
Temp->Active=true;
if (Temp->RecordCount!=0)
{
AnsiString A="Вы уверены?\nУдаляемую запись использует "+IntToStr(Temp->RecordCount)+" аспектов!";
int Q=Application->MessageBoxA(A.c_str(),"Удаление ситуации",MB_YESNO);
if(Q==IDYES)
{
Sit->Delete();
}
