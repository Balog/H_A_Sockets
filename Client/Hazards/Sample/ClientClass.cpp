        ...
        else
        {
                if(ToCopy->RecordCount()==FromCopy->RecordCount)
                {
                        ...
                        for(FromCopy->First();!FromCopy->Eof;FromCopy->Next())
                        {
                                ...
                                {
                                        Variant V=FromCopy->FieldList->Fields[i]->Value;
                                        String V2=FromCopy->FieldList->Fields[i]->AsString;
                                        ...
                                        {
                                        String TC=(String)ToCopy->Fields(i);
                                        if(TC!=V2)
                                        {
                                                //ShowMessage(FromCopy->FieldList->Fields[i]->DisplayName);
                                                //ShowMessage(V);
                                                //ShowMessage(ToCopy->Fields(i));
...
for(;i<200;i++)
{
Server.OleProcedure("MergeAspectsMainSpecH", this->IDC(), &Ret);
if(Ret)
{
break;
}
...
for(;i<200;i++)
{
Server.OleProcedure("PrepareLoadAspectsH", "Опасности", this->IDC(), Podr, Memo1Width, Memo2Width, Memo3Width, Memo4Width, &Ret);
if(Ret)
{
break;
}
...
for(;i<200;i++)
{
Server.OleProcedure("PrepareMergeAspectsH", "Опасности", this->IDC(), NumLogin, W1, W2, W3, W4, &Ret);
if(Ret)
{
break;
}
