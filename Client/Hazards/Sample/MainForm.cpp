...
Zast->MClient->WriteDiaryEvent("Hazards","Деморежим",Login);

}
else
{
Zast->MClient->WriteDiaryEvent("Hazards","Пользователь",Login);
...
Zast->MClient->WriteDiaryEvent("Hazards","Запуск модуля пользователя завершен","");
}
//---------------------------------------------------------------------------
...
Zast->MClient->WriteDiaryEvent("Hazards","Начало проверки необходимости загрузки справочников (автоматическое)","");
try
{
FProg->Progress->Position=0;
FProg->Progress->Min=0;
FProg->Progress->Max=6;
FProg->Visible=true;
...
FProg->Progress->Position++;
FProg->Label1->Caption="Чтение справочника участков/установок...";
FProg->Label1->Repaint();
if(VerifyTerr())
{
Ret=Ret & LoadTerr();
...
FProg->Progress->Position++;
FProg->Label1->Caption="Чтение справочника объектов оценки...";
FProg->Label1->Repaint();
if(VerifyDeyat())
{
Ret=Ret & LoadDeyat();
...
FProg->Progress->Position++;
FProg->Label1->Caption="Чтение справочника последствий...";
FProg->Label1->Repaint();
if(VerifyVozd())
{
Ret=Ret & LoadVozd();
...
Zast->MClient->WriteDiaryEvent("Hazards","Конец проверки необходимости загрузки справочников (автоматическое)","");
return Ret;
}
//---------------------------------------------------------
...
EnabledForm(true);
 ...
 N5=Aspects->FieldByName("Приоритетность")->AsInteger;
 N6=Aspects->FieldByName("Приор")->AsInteger;



Podrazdel->Active=false;
Podrazdel->CommandText="Select * From Подразделения";
Podrazdel->Active=true;
CPodrazdel->Clear();
CPodrazdel->ItemIndex=-1;
for(int i=0;i<Podrazdel->RecordCount;i++)
{
 CPodrazdel->Items->Add(Podrazdel->FieldByName("Название подразделения")->Value);

 if (N1==Podrazdel->FieldByName("Номер подразделения")->Value)
 {
 CPodrazdel->ItemIndex=i;
...
Situaciya->Active=false;
Situaciya->CommandText="Select * From Ситуации Where Показ=true";
Situaciya->Active=true;
CSit->Clear();
CSit->ItemIndex=-1;
CSit->Text="Выбери ситуацию";
for(int i=0;i<Situaciya->RecordCount;i++)
{
 CSit->Items->Add(Situaciya->FieldByName("Название ситуации")->Value);
 if (N2==Situaciya->FieldByName("Номер ситуации")->Value)
 {
 CSit->ItemIndex=i;
 }
 Situaciya->Next();
...
Posledstvie->Active=false;
Posledstvie->CommandText="Select * From [Проявление воздействия] Order by [Балл]";
Posledstvie->Active=true;
CProyav->Clear();
for(int i=0;i<Posledstvie->RecordCount;i++)
{
 CProyav->Items->Add(Posledstvie->FieldByName("Наименование проявления")->Value);
  if (N3==Posledstvie->FieldByName("Номер проявления")->Value)
 {
 CProyav->ItemIndex=i;
 }
 Posledstvie->Next();
...
Tiagest->Active=false;
Tiagest->CommandText="Select * From [Тяжесть последствий] Order by [Балл]";
Tiagest->Active=true;
CPosl->Clear();
CPosl->ItemIndex=-1;
for(int i=0;i<Tiagest->RecordCount;i++)
{
 CPosl->Items->Add(Tiagest->FieldByName("Наименование последствия")->Value);
 if (N4==Tiagest->FieldByName("Номер последствия")->Value)
 {
  CPosl->ItemIndex=i;
 }
 Tiagest->Next();
...
Prioritet->Active=true;
 CPrior->Clear();
 CPrior->ItemIndex=-1;
for(int i=0;i<Prioritet->RecordCount;i++)
{
 CPrior->Items->Add(Prioritet->FieldByName("Наименование приоритетности")->Value);
 if (N5==Prioritet->FieldByName("Номер приоритетности")->Value)
 {
  CPrior->ItemIndex=i;
 }
 Prioritet->Next();
...
CBPrior->Clear();
CBPrior->ItemIndex=-1;
for(int i=0;i<Prior->RecordCount;i++)
{
CBPrior->Items->Add(Prior->FieldByName("Наименование приоритетности")->Value);
 if (N6==Prior->FieldByName("Номер приоритетности")->Value)
 {
  CBPrior->ItemIndex=i;
 }
 Prior->Next();
...
{
Territoriya->Active=false;
Territoriya->CommandText="Select * From Территория Where [Номер территории]=0";
Territoriya->Active=true;
}
Edit1->Text=Territoriya->FieldByName("Наименование территории")->Value;
...
{
Deyatelnost->Active=false;
Deyatelnost->CommandText="Select * From Деятельность Where [Номер деятельности]=0";
Deyatelnost->Active=true;
}
Edit2->Text=Deyatelnost->FieldByName("Наименование деятельности")->Value;
...
{
Aspect->Active=false;
Aspect->CommandText="Select * From Аспект Where [Номер аспекта]=0";
Aspect->Active=true;
}
Edit4->Text=Aspect->FieldByName("Наименование аспекта")->Value;
...
{
Vozdeystvie->Active=false;
Vozdeystvie->CommandText="Select * From Воздействия Where [Номер воздействия]=0";
Vozdeystvie->Active=true;
}
Edit5->Text=Vozdeystvie->FieldByName("Наименование воздействия")->Value;
...
{
TempAspects->Active=false;
TempAspects->CommandText="Select * From Аспекты Where [Номер аспекта]="+DBEdit1->Text;
TempAspects->Active=true;
if(TempAspects->RecordCount!=0)
{
if(TempAspects->FieldByName("G")->AsBoolean)
{
...
if(TempAspects->FieldByName("R")->AsBoolean)
{
...
if(TempAspects->FieldByName("T")->AsBoolean)
{
...
if(TempAspects->FieldByName("L")->AsBoolean)
{
...
if(TempAspects->FieldByName("O")->AsBoolean)
{
...
if(TempAspects->FieldByName("S")->AsBoolean)
{
...
if(TempAspects->FieldByName("N")->AsBoolean)
{
...
//int NR,CR;
//NR=StrToInt(NumRecord->Text);
//CR=StrToInt(CountRecord->Text);
//if(Zast->Registered==false)
//{
/*
...
// Aspects->FieldByName("Ситуация")->Value=Situaciya->FieldByName("Номер ситуации")->Value;
 ...
 Aspects->FieldByName("Проявление воздействия")->Value=Posledstvie->FieldByName("Номер проявления")->Value;
 Aspects->FieldByName("Приоритетность")->Value=Prioritet->FieldByName("Номер приоритетности")->Value;
 Aspects->FieldByName("Приор")->Value=Prior->FieldByName("Номер приоритетности")->Value;
 Aspects->FieldByName("Предлагаемые мероприятия")->Value="";
 Aspects->FieldByName("Дата создания")->Value=Now();
 Aspects->FieldByName("Начало действия")->Value=Now();
 Aspects->FieldByName("Конец действия")->Value=Now();
...
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Сбой сохранения рисков","");
 ShowMessage("Ошибка записи рисков");
  Action=caNone;
}
}
else
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Сбой подготовки к записи рисков","");
 ShowMessage("Ошибка подготовки записи рисков");
  Action=caNone;
}
Zast->MClient->Stop();
...
Prioritet->First();
Prioritet->MoveBy(CPrior->ItemIndex);
VVR=Prioritet->FieldByName("Балл")->Value;
...
int NumRisk=Aspects->FieldByName("Номер аспекта")->Value;
TempAspects->Active=false;
TempAspects->CommandText="Select * From Аспекты Where [Номер аспекта]="+IntToStr(NumRisk);
TempAspects->Active=true;
TempAspects->Edit();
...
Znachimost->Active=false;
Znachimost->CommandText="Select * From Значимость Order By [Мин граница]";
Znachimost->Active=true;
AnsiString NZ;
AnsiString M;
int NumZn=-1;
...
double K1=Znachimost->FieldByName("Мин граница")->AsFloat;
double K2=Znachimost->FieldByName("Макс граница")->AsFloat;
if (p3>=K1 & p3<K2)
{
NumZn=Znachimost->FieldByName("Номер значимости")->Value;
NZ=Znachimost->FieldByName("Наименование значимости")->Value;
...
TempAspects->Edit();
TempAspects->FieldByName("Значимость")->Value=Zn;
TempAspects->FieldByName("Наименование значимости")->Value=NZ;
TempAspects->Post();
//DBCheckBox1->Checked=Zn;

CBPrior->ItemIndex=Aspects->FieldByName("Приоритетность")->AsInteger;
...
Zast->MClient->WriteDiaryEvent("Hazards","Обновление рисков (ручное)","");
...
InitCombo();
Zast->MClient->WriteDiaryEvent("Hazards","Обновление рисков (ручное) завершено","");
 ShowMessage("Завершено");
}
else
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Сбой обновления рисков (ручное)","");
 ShowMessage("Ошибка загрузки рисков");
}
Zast->MClient->Stop();
}
catch(...)
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Ошибка обновления рисков (ручное)"," Ошибка "+IntToStr(GetLastError()));
}
}
//---------------------------------------------------------------------------
...
{
bool Ret=true;
Zast->MClient->WriteDiaryEvent("Hazards","Начало загрузки рисков","");
try
{
...
Zast->MClient->WriteDiaryEvent("Hazards","Загрузка рисков подразделения","Подразделение: "+LPodr->FieldByName("Название подразделения")->AsString);
...
RTemp->Active(false);
RTemp->Active(true);
FProg->Label1->Caption="Загрузка рисков "+LPodr->FieldByName("Название подразделения")->AsString;
...
Table* LMemo1=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Table* LMemo2=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Table* LMemo3=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Table* LMemo4=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Zast->MClient->WriteDiaryEvent("Hazards","Загрузка рисков завершена","");
...
if(!Ret)
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Сбой обновления рисков","");
...
{
Zast->MClient->WriteDiaryEvent("Hazards","Подготовка к загрузке рисков","");
try
{
...
Zast->MClient->WriteDiaryEvent("Hazards","Подготовка к загрузке рисков завершена","");

}
catch(...)
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Ошибка подготовки к загрузке рисков"," Ошибка "+IntToStr(GetLastError()));
...
{
Zast->MClient->WriteDiaryEvent("Hazards","Обновление рисков","");
try
{
...
for(TempAsp->First();!TempAsp->Eof;TempAsp->Next())
{
 ...
 {
 Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Сбой обновления рисков","");
  ShowMessage("Ошибка объединения рисков");
 }
}
...
Table* Remote=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Ошибка проверки рисков"," Ошибка "+IntToStr(GetLastError()));
}
return Ret;
}
//-------------------------------------------
...
try
{
Zast->MClient->WriteDiaryEvent("Hazards","Начало загрузки ситуаций","");
...
Table* Sit=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
{
MergeSit();
Zast->MClient->WriteDiaryEvent("Hazards","Конец загрузки ситуаций","");
Ret=true;
}
...
if(!Ret)
{
Zast->MClient->WriteDiaryEvent("Hazards","Сбой загрузки ситуаций","");
}
}
catch(...)
{
...
Zast->MClient->WriteDiaryEvent("Hazards","Начало объединения ситуаций","");
...
Tab->CommandText="select * from Ситуации";
Tab->Active=true;
...
{
 int Num=Tab->FieldByName("Номер ситуации")->Value;

 if(Temp->Locate("Номер ситуации",Num,SO))
 {
  Tab->Edit();
  Tab->FieldByName("Название ситуации")->Value=Temp->FieldByName("Название ситуации")->Value;
  Tab->FieldByName("Показ")->Value=true;
  Tab->Post();
...
Comm->CommandText="DELETE Ситуации.Del FROM Ситуации WHERE Ситуации.Del=True AND Показ=True;";
Comm->Execute();
...
Comm->CommandText="DELETE TempSit.* FROM TempSit;";
Comm->Execute();
*/
}
catch(...)
...
{
bool Ret=false;
Zast->MClient->WriteDiaryEvent("Hazards","Начало загрузки территорий","");
try
{
...
Table* Terr=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Zast->MClient->WriteDiaryEvent("Hazards","Конец загрузки территорий","");
Ret=true;
}
...
{
Zast->MClient->WriteDiaryEvent("Hazards","Начало объединения территорий","");
try
{
MP<TADOCommand>Comm(this);
Comm->Connection=Zast->ADOUsrAspect;
...
{
bool Ret=false;
Zast->MClient->WriteDiaryEvent("Hazards","Начало загрузки деятельностей","");
try
{
...
Table* Deyat=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Zast->MClient->WriteDiaryEvent("Hazards","Конец загрузки деятельностей","");
...
{
Zast->MClient->WriteDiaryEvent("Hazards","Начало объединения деятельностей","");
try
{
MP<TADOCommand>Comm(this);
Comm->Connection=Zast->ADOUsrAspect;
...
Table* Asp=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Zast->MClient->WriteDiaryEvent("Hazards","Конец загрузки опасностей","");
Ret=true;
}
...
if(!Ret)
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Сбой загрузки опасностей","");
}
}
catch(...)
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Ошибка загрузки опасностей"," Ошибка "+IntToStr(GetLastError()));
}
return Ret;
}
//---------------------------------------------
...
{
Zast->MClient->WriteDiaryEvent("Hazards","Начало объединения опасностей","");
try
{
MP<TADOCommand>Comm(this);
Comm->Connection=Zast->ADOUsrAspect;
...
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Ошибка объединения опасностей"," Ошибка "+IntToStr(GetLastError()));
...
{
bool Ret=false;
Zast->MClient->WriteDiaryEvent("Hazards","Начало загрузки воздействий","");
try
{
...
Table* Vozd=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Zast->MClient->WriteDiaryEvent("Hazards","Конец загрузки воздействий","");
Ret=true;
}
...
{
Zast->MClient->WriteDiaryEvent("Hazards","Начало объединения воздействий","");
try
{
MP<TADOCommand>Comm(this);
Comm->Connection=Zast->ADOUsrAspect;
...
{
bool Ret=false;
Zast->MClient->WriteDiaryEvent("Hazards","Начало загрузки критериев","");
try
{
...
Table* Zn=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Zast->MClient->WriteDiaryEvent("Hazards","Конец загрузки критериев","");
Ret=true;
}
...
{
Zast->MClient->WriteDiaryEvent("Hazards","Начало объединения критериев","");
try
{
MP<TADOCommand>Comm(this);
Comm->Connection=Zast->ADOUsrAspect;
...
{
Zast->MClient->Start();
Zast->MClient->WriteDiaryEvent("Hazards","Начало обновления справочников (ручное)","");
try
{
...
SetCombo();
InitCombo();
Zast->MClient->WriteDiaryEvent("Hazards","Конец обновления справочников (ручное)","");
 ShowMessage("Завершено");
}
else
{
...
Zast->MClient->Start();
Zast->MClient->WriteDiaryEvent("Hazards","Начало записи рисков (ручное)","");
try
{
DataSetRefresh1->Execute();
DataSetPost1->Execute();
...
if(SaveAspects())
{
Zast->MClient->WriteDiaryEvent("Hazards","Конец записи рисков (ручное)","");
 ShowMessage("Завершено");
}
else
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Сбой записи рисков (ручное)","");
...
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Сбой подготовки к записи рисков (ручное)","");
...
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Ошибка записи рисков (ручное)"," Ошибка "+IntToStr(GetLastError()));
...
{
Zast->MClient->WriteDiaryEvent("Hazards","Начало подготовки к записи рисков","");
try
{
MP<TADOCommand>Comm(this);
Comm->Connection=Zast->ADOUsrAspect;
...
CT=CT+" SELECT Аспекты.[Номер аспекта], Аспекты.Подразделение, Аспекты.Ситуация, Аспекты.[Вид территории], Аспекты.Деятельность, Аспекты.Специальность, Аспекты.Аспект, Аспекты.Воздействие, Аспекты.G, Аспекты.O, Аспекты.R, Аспекты.S, Аспекты.T, Аспекты.L, Аспекты.N, Аспекты.Z, Аспекты.Значимость, Аспекты.[Наименование значимости], Аспекты.[Проявление воздействия], Аспекты.[Тяжесть последствий], Аспекты.Приоритетность, Аспекты.[Выполняющиеся мероприятия], Аспекты.[Предлагаемые мероприятия], Аспекты.[Мониторинг и контроль], Аспекты.[Предлагаемый мониторинг и контроль], Аспекты.Исполнитель, Аспекты.[Дата создания], Аспекты.[Начало действия], Аспекты.[Конец действия], Аспекты.[ServerNum] ";
...
Zast->MClient->WriteDiaryEvent("Hazards","Конец подготовки к записи рисков","");

return true;
}
catch(...)
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Ошибка подготовки к записи рисков"," Ошибка "+IntToStr(GetLastError()));
...
{
bool Ret=false;
Zast->MClient->WriteDiaryEvent("Hazards","Начало записи рисков","");
try
{
...
Table* STemp=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);

STemp->SetCommandText("SELECT TempAspects.[Номер аспекта], TempAspects.Подразделение, TempAspects.Ситуация, TempAspects.[Вид территории], TempAspects.Деятельность, TempAspects.Специальность, TempAspects.Аспект, TempAspects.Воздействие, TempAspects.G, TempAspects.O, TempAspects.R, TempAspects.S, TempAspects.T, TempAspects.L, TempAspects.N, TempAspects.Z, TempAspects.Значимость, TempAspects.[Наименование значимости], TempAspects.[Проявление воздействия], TempAspects.[Тяжесть последствий], TempAspects.Приоритетность, TempAspects.Приор, TempAspects.[Дата создания], TempAspects.[Начало действия], TempAspects.[Конец действия], TempAspects.ServerNum FROM TempAspects ORDER BY TempAspects.[Номер аспекта];");
STemp->Active(true);
FProg->Label1->Caption="Сохранение опасностей";
...
Table* ST=Zast->MClient->CreateTable(this, Zast->ServerName, Zast->VDB[Zast->GetIDDBName("Опасности")].ServerDB);
...
Zast->MClient->WriteDiaryEvent("Hazards","Конец записи рисков","");
}
catch(...)
{
Zast->MClient->WriteDiaryEvent("NetAspects ошибка","Ошибка записи рисков"," Ошибка "+IntToStr(GetLastError()));
...
{
bool Ret=false;
Zast->MClient->WriteDiaryEvent("Hazards","Начало загрузки подразделений","");
try
{
MP<TADODataSet>Temp(this);
Temp->Connection=Zast->ADOUsrAspect;
...
Comm->Execute();
Zast->MClient->WriteDiaryEvent("Hazards","Конец загрузки подразделений","");
Ret=true;
}
...
if(!Ret)
{
Zast->MClient->WriteDiaryEvent("Hazards","Сбой загрузки подразделений","");
}
}
catch(...)
{
