...
{
Zast->MClient->WriteDiaryEvent("Hazards","Начало загрузки методики","");
try
{
if(LoadMet())
{
...
ADODataSet1->Active=true;
Zast->MClient->WriteDiaryEvent("Hazards","Конец загрузки методики","");
return "Завершено";

}
else
{
Zast->MClient->WriteDiaryEvent("Hazards","Сбой загрузки методики","");
return "Ошибка";
}
}
catch(...)
